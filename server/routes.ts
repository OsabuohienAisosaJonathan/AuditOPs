...
export async function registerRoutes(
  ...
  // const PgStore = connectPgSimple(session); // Removed
  ...
  if (isProduction) {
    // Production: Using MemoryStore for simplicity in this migration (Go54/cPanel)
    // Note: Sessions will clear on restart. For persistence, use express-mysql-session.
    sessionStore = new MemoryStoreFactory({
      checkPeriod: 86400000,
    });
    console.log("[Session Store] Using MemoryStore (production - simplified for migration)");
  } else {
  ...
  if (isProduction) {
    // Session pruning not needed for MemoryStore (handled internally)
  }
